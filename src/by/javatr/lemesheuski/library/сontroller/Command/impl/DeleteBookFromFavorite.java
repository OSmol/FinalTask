...
public class DeleteBookFromFavorite implements Command {
    @Override
    public String execute(String request) {
        ...
        if (!type.equals("")) {
            if (requestParams.length == 4) {
                ...
                try {
                    bookService.deleteBookFromFavorites(username, title, author);
                    response = type + "&" + username + "&Book successfully removed from favorite";
                } catch (ServiceException e) {
                    response = type + "&" + username + "&Removing error " + e.getMessage();
                }
            } else
